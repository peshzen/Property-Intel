...
import { json, parseJsonBody } from './_auth';
...
export const handler: Handler = async (event) => {
  ...
  if (profileError || !profile || profile.approval_status !== 'approved') {
    return json(403, { error: 'User is not approved.' });
  }
  ...
  if (body.mode === 'geocode') {
    ...
    if (!key) return json(200, { warning: 'Google Maps key unavailable. Skipping geocoding.' });
    ...
    if (!result) return json(404, { error: 'Address not found' });
    const comps: Record<string, string> = {};
    for (const c of result.address_components ?? []) for (const t of c.types) comps[t] = c.long_name;
    return json(200, { normalizedAddress: result.formatted_address, city: comps.locality ?? '', county: comps.administrative_area_level_2 ?? '', state: comps.administrative_area_level_1 ?? '', zip: comps.postal_code ?? '', latitude: result.geometry.location.lat, longitude: result.geometry.location.lng });
  }
