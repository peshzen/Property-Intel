...
export const handler: Handler = async (event) => {
  ...
  const { error } = await serviceClient
    .from('profiles')
    .update({ google_maps_api_key_encrypted: null, google_maps_api_key_status: 'not_connected', google_maps_api_key_last_tested_at: null })
    .eq('id', authData.user.id);

  if (error) return { statusCode: 500, body: JSON.stringify({ error: error.message }) };
  return { statusCode: 200, body: JSON.stringify({ ok: true, status: 'not_connected' }) };
};
